// v1.9.213: 방(room) 시스템 — 로그인 직후엔 로비 화면 없이 곧장 "모두의 방"(DEFAULT_ROOM_ID)에
// 입장한다. 방 목록은 게임 중 "n명 접속 중" 모달의 "방 목록" 버튼으로 여는 다른 모달들과 같은
// 스타일의 팝업이고(배경을 눌러 닫아도 지금 있는 방에서 나가지지 않음), 방 인원수 제한은
// 없다(모두의 방/일반 방 모두 무제한). "나가기"는 지금 있는 방이 모두의 방이 아닐 때만 보이고,
// 누르면 모두의 방으로 돌아간다.
const fs = require('fs');
...
setTimeout(async () => {
  ...
  results.push(['로그인 직후: 방 목록 모달은 안 보임', lobbyOverlayEl.classList.contains('hidden')]);
  ...
  // --- "n명 접속 중" 모달 — 모두의 방에 있을 땐 "나가기" 버튼이 안 보임(더 나갈 곳이 없음) ---
  userCountTagEl.click();
  await new Promise(r => setTimeout(r, 20));
  results.push(['모두의 방에서는 "나가기" 버튼이 숨겨짐', leaveRoomBtnEl.classList.contains('hidden')]);

  // --- "방 목록" 버튼 — 다른 모달과 같은 스타일로 뜨고, 지금 있는 방에서 나가지지 않음 ---
  browseRoomsBtnEl.click();
  await new Promise(r => setTimeout(r, 30));
  results.push(['"방 목록" 버튼을 눌러도 모두의 방에 그대로 있음', currentRoomId === DEFAULT_ROOM_ID]);
  results.push(['방 목록 모달(배경+카드)이 보임', !lobbyOverlayEl.classList.contains('hidden') && !lobbyBackdropEl.classList.contains('hidden')]);

  // --- 방 목록에 기본방 + 기존 방 2개, 총 3개가 표시됨 ---
  ...
  results.push(['인원이 많은 방도 인원수만 표시되고 막히지 않음(무제한)', !!roomManyLi && roomManyLi.querySelector('.lobbyRoomCount').textContent === '37명' && !roomManyLi.classList.contains('full')]);

  // --- 배경(백드롭)을 눌러 닫아도 지금 있는 방(모두의 방)에서 나가지지 않음 ---
  lobbyBackdropEl.click();
  await new Promise(r => setTimeout(r, 20));
  results.push(['배경 클릭으로 방 목록만 닫힘', lobbyOverlayEl.classList.contains('hidden')]);
  ...
  results.push(['입장하면 방 목록 모달이 닫힘', lobbyOverlayEl.classList.contains('hidden')]);
  results.push(['입장하면 내 프레즌스가 그 방에 올라감', !!window.__STORE.roomPresence.roomA.M0]);

  // --- "n명 접속 중" 모달 — 모두의 방이 아닌 방에 있을 땐 "나가기" 버튼이 보임 ---
  userCountTagEl.click();
  await new Promise(r => setTimeout(r, 20));
  results.push(['모두의 방이 아니면 "나가기" 버튼이 보임', !leaveRoomBtnEl.classList.contains('hidden')]);

  // --- "방 목록" 버튼 — 로비를 다시 열되, 아직 방을 나간 건 아님(구경만) ---
  browseRoomsBtnEl.click();
  await new Promise(r => setTimeout(r, 30));
  results.push(['"방 목록" 버튼: 모달이 다시 열림', !lobbyOverlayEl.classList.contains('hidden')]);
  results.push(['"방 목록" 버튼: 아직 roomA에 그대로 있음(구경만)', currentRoomId === 'roomA' && !!window.__STORE.roomPresence.roomA.M0]);

  // 방 목록에서 지금 있는 방(roomA)을 다시 누르면 그냥 복귀만 함
  const roomAAgain = [...lobbyRoomListEl.querySelectorAll('li')].find(li => li.querySelector('.lobbyRoomTitle').textContent === '초보만 오세요');
  roomAAgain.click();
  await new Promise(r => setTimeout(r, 30));
  results.push(['같은 방을 또 누르면 그냥 복귀(모달 닫힘, 여전히 roomA)', lobbyOverlayEl.classList.contains('hidden') && currentRoomId === 'roomA']);
